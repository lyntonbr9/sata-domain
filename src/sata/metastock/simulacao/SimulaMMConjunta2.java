 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class SimulaMMConjunta2 {
		 ...
		 */
			...
			/*		  mm = new MediaMovel(close,mmMai); 
				  ...
				  for(int i=0;i<ptoCompras[0].length ;i++){
						   ...
						   if(acao!=-1 && ptoCompras[acao][i] ){
						   	
						   		if(i+1 < high[acao].length){	
						   			diasInvestidos++;
							   	
				   			    	if(getGanho(close[acao][i],high[acao][i+1])>=0.01){
				   			    		
				   			    		ganho += 0.01;
						   			    positivas++;
				   			    		
				   			    		
				   			    	}else if(getGanho(close[acao][i],close[acao][i+1])>=0.00){
				   			    		ganho += getGanho(close[acao][i],close[acao][i+1]);
				   			    		positivas++;
				   			    	}else{
				   			    		ganho += (-1) * getPerda(close[acao][i],close[acao][i+1]);
						   			    negativas++;
				   			    	}
				   			    	
					   			    ganhoTotal += (ganho - 0.0032);
						   	...
						   	//		    System.out.println(("" + ganhoTotal).replace('.',',') + " 0," +ano);
						   			    System.out.println("Ganho: " + ganho + " ação: " + acoes[acao] +" Ganho Total:" + ganhoTotal + 
						   			    		" data Compra:" + datas[acao][i] + " data venda:" + datas[acao][i] + 
						   						" mm1:" + close[acao][i-1] +" mm2:" + mmMenor[acao][i-1] + " mm1:" + close[acao][i] +" mm2:" + mmMenor[acao][i]); 
