...
//TODO: Atualmente é utilizado o Hibernate
public class MySQLCotacaoOpcaoDAO implements ICotacaoOpcaoDAO, IConstants {

	private Connection con;
	
	@Deprecated
	public List<CotacaoOpcaoTO> getCotacoesDaOpcao(String codigoOpcao) {
//		List<CotacaoOpcaoTO> listaCotacoesDaOpcao = new ArrayList<CotacaoOpcaoTO>();
//		String sqlStmt = "SELECT * FROM CotacaoOpcao WHERE "
//			+ " codigoAtivo = '" + codigoOpcao + "' "
...
//		return listaCotacoesDoAtivo;
	...
	public int getSplit(CotacaoOpcaoTO caTO){
...
//		System.out.println("periodoFormatadoParaBD= " + periodoFormatadoParaBD + " resultado split: " + resultado);
	...
	public List<CotacaoOpcaoTO> getCotacoesDaOpcao(String codigoOpcao, String dataInicial, 
													String dataFinal) {
...
//		return listaCotacoesDoAtivo;
	...
	public void insertCotacaoDaOpcao(CotacaoOpcaoTO caTO) {
		
//		String tabela = "CotacaoOpcao"; //tabela das acoes
...
//		}
	...
	public boolean existeCotacao(String codigoOpcao, String periodo){
		
		String sqlStmt = "SELECT codigoAtivo, periodo FROM CotacaoOpcao"
			+ " WHERE codigoAtivo = ? AND periodo = ? ";
		
		try {
			PreparedStatement ps = con.prepareStatement(sqlStmt);
			ps.setString(1, codigoOpcao);
	...
	@Override
	public int updateCotacaoDaOpcao(CotacaoOpcaoTO caTO) {
		
		String sqlStmt = "UPDATE CotacaoOpcao SET " +
...
//		System.out.println(resultadoOperacao);
	...
	public static String analisaMelhorOpcaoCompra(CotacaoOpcaoTO cotacaoAcaoTO, int indiceSerieOpcao, String dataInicial, String dataFinal) throws SQLException{

		ICotacaoOpcaoDAO caDAO = SATAFactoryFacade.getCotacaoOpcaoDAO();
		...
		for(int i = valorAcaoArredondado; i >= valorAcaoArredondado - 4; i--)
		{
			...
			List<CotacaoOpcaoTO> listaTodasCotacoesOpcaoPUT = caDAO.getCotacoesDaOpcao(nomeOpcao, SATAUtil.getDataFormatadaParaBD(dataInicial), SATAUtil.getDataFormatadaParaBD(dataFinal));
			...
			List<CotacaoOpcaoTO> listaCotacaoOpcaoCorrente = caDAO.getCotacoesDaOpcao(nomeOpcao, SATAUtil.getDataFormatadaParaBD(cotacaoAcaoTO.getPeriodo()), SATAUtil.getDataFormatadaParaBD(cotacaoAcaoTO.getPeriodo()));
			if(listaCotacaoOpcaoCorrente.size() == 0)
			{
				System.out.println("###################");
				continue;
			}
			CotacaoOpcaoTO cotacaoOpcaoCorrente = listaCotacaoOpcaoCorrente.get(0);
			int j=0;
			for(CotacaoOpcaoTO caTO : listaTodasCotacoesOpcaoPUT){
				j++;
...
//					System.out.println("valorOpcao: " + valorOpcao);
		...
		for(int i = valorAcaoArredondado; i <= valorAcaoArredondado + 4; i++)
		{
			...
			System.out.println("nomeOpcao: " + nomeOpcao);
			//pega todas as cotacoes da opcao de todos os dias
			List<CotacaoOpcaoTO> listaTodasCotacoesOpcaoCall = caDAO.getCotacoesDaOpcao(nomeOpcao, SATAUtil.getDataFormatadaParaBD(dataInicial), SATAUtil.getDataFormatadaParaBD(dataFinal));
			...
			CotacaoOpcaoTO cotacaoOpcaoCorrente = listaCotacaoOpcaoCorrente.get(0);
			int j=0;
			for(CotacaoOpcaoTO caTO : listaTodasCotacoesOpcaoCall){
				j++;
				if(cotacaoOpcaoCorrente.getPeriodo().equalsIgnoreCase(caTO.getPeriodo())){ //Porque existem dias que a opcao nao teve cotacao(negociacao)
					System.out.println(caTO.getPeriodo());
					System.out.println("F: " + caTO.getFechamento());
...
//		MySQLDAOFactory.returnConnection(con);
	...
	private MySQLCotacaoOpcaoDAO(Connection connection){
		this.con = connection;
	}
	private static MySQLCotacaoOpcaoDAO instance;
	public static MySQLCotacaoOpcaoDAO get(Connection connection) {	
		if (instance != null) {
	...
	private static synchronized MySQLCotacaoOpcaoDAO create(Connection connection) {
		if (instance == null) instance = new MySQLCotacaoOpcaoDAO(connection);
		return instance;
	}
}
